...
coor * Coor_init () {
	FILE * in = fopen ("coor.txt", "r");
		...
		fscanf (in, "%lf%lf%lf%lf%lf%lf%lf%lf", &(res->x1), &(res->y1), &(res->x2), &(res->y2), &(res->x3), &(res->y3), &(res->x4), &(res->y4));
...
step * Step_init (coor * Coor) {
	step * Step = (step *) malloc (sizeof(step));
		Step->up_x = (Coor->x1 - Coor->x3) / Coor->height;
		Step->up_y = (Coor->y1 - Coor->y3) / Coor->height;
		Step->right_x = (Coor->x4 - Coor->x3) / Coor->width;
		Step->right_y = (Coor->y4 - Coor->y3) / Coor->width;
	return Step;
}

void point_to_real_coor (track_point * point, coor * Coor, step * Step) {
		point->x = Coor->x3 + point->pic_x * Step->right_x + point->pic_y * Step->up_x;
		point->y = Coor->y3 + point->pic_x * Step->right_y + point->pic_y * Step->up_y;
	return;
}	
...
int main () {
	...
	FILE * in = fopen ("coordinates.txt", "r");				// файл с координатами траектории
	...
	step * Step = Step_init (Coor);	
		
		printf ("\n	1) %lf %lf	2) %lf %lf\n	3) %lf %lf	4) %lf %lf\n\n", Coor->x1, Coor->y1, Coor->x2, Coor->y2, Coor->x3, Coor->y3, Coor->x4, Coor->y4);
		
		cur = points->head;
		for (i=0; i<count_points; i++) {
			point_to_real_coor (cur, Coor, Step);
			cur = cur->next;
		}
		print_point (points);
