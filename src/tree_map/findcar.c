...
in=fopen(fbmp1,"rb"); 
out=fopen(fbmp2,"wb");
mas=(unsigned char*)malloc(Width*Height*4);
    if(mas==NULL)
	{
...
for(j=0;j<Width; j++){
  ...
  G = (D/256)%256;
  B = (D/256/256)%256;
   mas[i*Width*4+4*j+0]= B;
   mas[i*Width*4+4*j+1]= G;
   mas[i*Width*4+4*j+2]= R;
 }                                                       
}
printf("%d %d\n", Height, Width);
fread((void*) bm,1,54,in);
...
/*int TR2FIELD(struct FIELD Field,struct TR Tr)
{
	...
	for(i=0;i<N;i++)
	{
		...
		//printf("%d %d\n", nw,nh);
		//printf("%d %d\n", width,height);
		if ((nw>=width)||(nh>=height)) continue;//изменение с 3 на 4!!!!!!!!!!!!!!!!
		else
		{
			field[nh*width*3+nw*3+0]=0*b;
			field[nh*width*3+nw*3+1]=0*r;
			field[nh*width*3+nw*3+2]=0*g;
		}
	}
	return 0;
}*/
